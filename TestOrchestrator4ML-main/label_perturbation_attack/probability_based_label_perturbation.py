...
logger = logging.getLogger("label_pert_attk/prob-pert")
...
def poisonous_data_perturbation(X_train, X_test, X_val, y_train, y_test, y_val, change_unit, I, g):    
    logger.info(f"poisonous_data_perturbation({X_train}, {X_test}, {y_train}, {y_test}"
                + "{y_val}, {change_unit}, {I}, {g})")
    try:     
        D_1_X = X_train
        D_1_y = y_train
    ...
    #         np.append(D_1_X, temp_D_1_X[len(D_1_X) + i_1])
        ...
        logger.error(f"poisonous_data_perturbation({X_train}, {X_test}, {y_train}, {y_test}"
                    + "{y_val}, {change_unit}, {I}, {g}) FAILED: {e}")
        raise e