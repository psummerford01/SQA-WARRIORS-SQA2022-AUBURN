...
logger = logging.getLogger("detect/main")
...
def get_test_details(test_script):
    logger.info(f"get_test_details({test_script})")
    try:
        test_name_list = []
...
#               print(the_assert_tup)
        ...
        logger.error(f"get_test_details({test_script}) FAILED: {e}")
        raise e
...
def checkClassificationAlgoTest(test_script):
    ...
    try:
        ...
        logger.error(f"checkClassificationAlgoTest({test_script}) FAILED: {e}")
        raise e
  
  
def checkAccuracyTest(test_script):
    logger.info(f"checkAccuracyTest({test_script})")
    try:
        py_tree = py_parser.getPythonParseObject(test_script)
        ...
        logger.error(f"checkAccuracyTest({test_script}) FAILED: {e}")
        raise e
    
    
def chackAttackTest(test_script, assert_list):
    logger.info(f"chackAttackTest({test_script}, {assert_list})")
    try:
        attack_check = []
        py_tree = py_parser.getPythonParseObject(test_script)
        ...
        logger.error(f"chackAttackTest({test_script}, {assert_list}) FAILED: {e}")
        raise e
...
def runDetectionTest(inp_dir, test_output_csv, test_assert_output_csv, flag_output_csv):
    ...
    try:
        ...
        logger.error(f"runDetectionTest({inp_dir},{test_output_csv},{test_assert_output_csv},{flag_output_csv}) FAILED: {e}")
        raise e
